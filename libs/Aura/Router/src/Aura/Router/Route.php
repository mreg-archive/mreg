...
class Route
{
    ...
    public function generate(array $data = null)
    {
        ...
        if ($this->generate) {
            //call_user_func needed for php 5.3
            $data = call_user_func_array($this->generate, array($this, (array) $data));
                //replaces
    ...
    protected function isCustomMatch($server)
    {
        ...
        // call_user_func needed for php 5.3
        $result = call_user_func_array($this->is_match, array($server, &$matches));
            // replaces
